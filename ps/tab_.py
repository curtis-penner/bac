...
class Tabformat:
    ...
    def __init__(self):
        ...
        self.rhstyle = constants.RHSIMPLE   # rhythm flag style
        ...
        self.brummer = constants.BRUMMER_ABC   # sytle for brumemr in german tab
    ...
    def read_tab_format(self, line):
        ...
        if key == "tabbrummer":
            if value == "ABC":
                self.brummer = constants.BRUMMER_ABC
                return True
            elif value == "1AB":
                self.brummer = constants.BRUMMER_1AB
                return True
            elif value == "123":
                self.brummer = constants.BRUMMER_123
                return True
            else:
                return False
    ...
    def is_tab_line(self, line):
        ...
        if self.is_tab_key(v_obj.switch_voice(voice_spec).key):
            return True
        else:
            # check default current voice
            if tab_key.is_tab_key(v_obj.key):
                return True
        return False
    ...
    def parse_tab_line(self, line):
        """
        parses a tablature line into music

        :param line:
        :return:
        """
        if not voice:
            log.critical("Trying to parse undefined voice")
            exit(-1)

        # check for mixing of tablature and music within one line
        voicespec, line = util.get_field_value('V', line)

        # voicename == voicespec[2]
        if not self.is_tab_key(voice[switch_voice(voicename)].key):
            print("+++ Error line %d: "
    ...
    def open_tabfontfile(self, basename):
        ...
        if not font_dirs:
            font_dirs = constants.TABFONTDIRS
    ...
    def def_tabfonts(self, fp):
        ...
        font_list = [self.front, self.itfont, self.defont]
