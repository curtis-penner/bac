...
void parse_tab_args(char** av, int* i)
{
  if (!strcmp(av[*i],"-tabsize")) {
    ...
    if (tabfont.size<=0) { error("illegal tablature fontsize: ",av[*i]); }
  } else
...
int read_tab_format (char* line)
{
  ...
  if (!strcmp(param,"tabfontsize")) {
    int fs = atoi(value);
    if (fs<=0)
      warning("illegal value for 'tabfontsize': ",value);
    else
      tabfont.size = fs;
  }
  if (!strcmp(param,"tabfontscale")) {
    float fs = atof(value);
    if (fs<=0.0)
      warning("illegal value for 'tabfontscale': ",value);
    else
      tabfont.scale = fs;
  }
  ...
  case FRENCH5TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 7) {
        ...
        bourdonwarning=1;
        warning("Warning: french5tab only supports 6 courses","");
      }
    }
    break;
  case FRENCH4TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 6) {
        ...
        bourdonwarning=1;
        warning("Warning: french4tab only supports 5 courses","");
      }
    }
    break;
  case SPANISHTAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 7) {
        ...
        bourdonwarning=1;
        warning("Warning: diapasons are ignored in spanishtab","");
      }
    }
    break;
  case SPANISH5TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 6) {
        ...
        bourdonwarning=1;
        warning("Warning: spanish5tab/banjo5tab only supports 5 courses","");
      }
    }
    break;
  case SPANISH4TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 5) {
        ...
        bourdonwarning=1;
        warning("Warning: spanish4tab/banjo4tab/ukuleletab only supports 4 courses","");
      }
    }
    break;
  case ITALIANTAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 7) {
        ...
        bourdonwarning=1;
        warning("Warning: diapasons are ignored in italiantab","");
      }
    }
    break;
  case ITALIAN7TAB:
  ...
  case ITALIAN5TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 6) {
        ...
        bourdonwarning=1;
        warning("Warning: italian5tab only supports 5 courses","");
      }
    }
    break;
  case ITALIAN4TAB:
    for (j=0;j<s->npitch;j++) {
      ...
      if (s->pits[j] < 5) {
        ...
        bourdonwarning=1;
        warning("Warning: italian4tab only supports 4 courses","");
      }
    }
    break;
  case GERMANTAB:
    {
      for (j=0;j<s->npitch;j++) {
        ...
        if (s->pits[j] < 7) {
          ...
          bourdonwarning=1;
          warning("Warning: diapasons are ignored in germantab","");
        }
      }
      break;
    }
...
void draw_tabdeco (float x, struct SYMBOL *s, float* gchy)
{
  ...
  for (i=0;i<s->dc.n;i++) {
	...
	case D_TABOLINE:
      if (italian)
        warning("Warning: decoration 'L' (oblique line) ignored in italiantab","");
      else if (german)
        warning("Warning: decoration 'L' (oblique line) ignored in germantab","");
      else
        PUT2("%.1f %.2f taboline\n", x, 
...
FILE* open_tabfontfile (char* basename)
{
  ...
  if ((envvar=getenv("ABCTABFONTS")))
	...
	fontdirs = strdup(TABFONTDIRS.c_str());
  ...
  dir = strtok(fontdirs,";");
  while (dir && !fontfp) {
	sprintf(fontfile,("%s" + DIRSEPCHAR + "%s.ps").c_str(),dir,basename);
...
void def_tabfonts (FILE *fp)
{
  ...
  if (strcmp(tabfont.frfont,"none")) {
	...
	if (!fontfp) {
      error("Please check the environment variable ABCTABFONTS\n"
