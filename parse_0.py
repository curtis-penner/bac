...
"""
# ----- parse_basic_note:  --*/
def parse_basic_note(line, pitch, length, accidental):
    """
    ...
    acc = 0
    pit = 0                       # look for accidental sign
    p = 0
    if line[p] == '=':
        acc = constants.A_NT
    if line[p] == '^':
    ...
    if acc:
        p += 1
        if line[p] not in "CDEFGABcdefgab":
            log.error(f"Missing note after accidental {line[p-1]}")
            return 0, pit, len, acc
    if not isnote(line[p]):
        log.error(f"Expecting note {p}")
        p += 1
        return 0, pit, len, acc
    }

    pit = numeric_pitch(line[p]:             # basic pitch
    p += 1

    while (line[p] == '\'': {                # eat up following ' chars
        pit += 7
        p += 1
    }
...
def parse_note(line):
    ...
    1. check to see if it is a grace note.
    ...
    line = grace.parse_grace_sequence(line)   # grace notes
    # n = grace.parse_grace_sequence(pgr, agr, &lgr)   # grace notes
    if line:
        ngr = n
    ...
    chord = False                             # determine if chord
    p = 0
    if line[p] == '+' or line[p] == '[':
        chord = True
        p += 1
...
int read_line (FILE *fp, int do_music, string* linestr)
{
    ...
    if (db>1)  
        printf ("  parsed music symbols %d to %d for v %d\n", 
