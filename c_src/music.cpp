...
int draw_keysig (float x, struct SYMBOL s)
{
  ...
  if (t==A_SH) {
    ...
    bug("wrong type in draw_keysig", false);
...
void draw_basic_note (float x, float w, float d, struct SYMBOL *s, int m)
{
  ...
  if (s->lens[0] >= LONGA) {
    ...
    else if (s->head==H_EMPTY) PUT1(" Hd%c", historic);
    else if (s->head==H_FULL) PUT1(" hd%c", historic);
  }
  if (s->shhd[m]) {
    yy=0;
...
float draw_note (float x, float w, float d, struct SYMBOL *s, int fl, float *gchy)
{
  ...
  for (m=0;m<s->npitch;m++) {                       
    ...
    if (!cfmt.nostems) {  /* stems can be suppressed */
      if (xx<0.01) {                                 /* add stem */
        if (s->stem) PUT3(" %.1f s%c%c",slen,c,historic);
        if (fl && (s->flags>0))                      /* add flags */
          PUT4(" %.1f f%d%c%c",slen,s->flags,c,historic);
      }
      if ((m>0) && (s->pits[m]==s->pits[m-1])) {     /* unions */
        if (s->stem) PUT3(" %.2f s%c%c",slen0,cc,historic);
        if (s->flags>0)                    
          PUT4(" %.1f f%d%c%c",slen0,s->flags,cc,historic);
      }
    }
  }
...
int vc_select (void)
{
  ...
  error("Bad voice selection: -V ", vcselstr);
  return 0;
}
...
void process_textblock(FILE *fpin, FILE *fp, int job)
{
  ...
  words_of_text.clear();
  for (i=0;i<100;i++) {
    if (feof(fpin)) { error("EOF reached scanning text block",""); }
    getline(&ln, fpin);
    ll=ln.length();
    linenum++;
...
void process_pscomment (FILE *fpin, FILE *fp, char *line)
{
  ...
  if (!strcmp(w,"begintext")) {
    ...
    if      (!strcmp(fstr,"obeylines")) job=OBEYLINES;
        ...
        job=SKIP; 
        error("bad argument for begintext: ",fstr); 
    }
    ...
    if (do_this_tune) {
      ...
      if (epsf) {
        ...
        if ((feps = fopen (fnm,"w")) == NULL) {
            error("Cannot open output file ", fnm);
        }
        init_ps (feps, finf, 1,
...
void process_file (FILE *fpin, FILE *fpout, char *xref_str, int npat, char (*pat)[STRLFILE], int sel_all, int search_field)
{
  ...
  type = read_line(fpin, false, &linestr);
