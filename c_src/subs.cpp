...
void init_ops (bool job)
{
  ...
  if (job) {
    strcpy (styd, DEFAULT_FDIR.c_str());
    ...
    if (av[i][0]=='+') {        /* switch off flags with '+' */
      ...
      if (!strncmp(av[i],"-tab",4) || 
        ...
        while (m<k) {
          ...
          if      (c=='h')  help_me=1;                 /* simple flags */
            ...
            nsel++;
            strcpy (sel_str[nsel], "");
            if (number_input_files==0) {                /* selector before first file */
      ...
      if (strstr(av[i],".fmt")) {     /* implict -F */ 
        ...
        if (!sel_arg) {             /* this arg is a file name */
          if (number_input_files>=MAXINF) {
            printf ("+++ Too many input files, max is %d\n", MAXINF);
            return 1;
          }
          strcpy (in_file[number_input_files], av[i]); 
          psel[number_input_files]=nsel;
          number_input_files++;
        }
...
void process_cmdline (char *line)
{
  ...
  if (!linecopy) { error("Out of memory",""); }
...
bool set_page_format(void)
{
  ...
  bool i = read_fmt_file ("fonts.fmt", styd, &cfmt);
  bool j = false;
  if (strlen(styf)>0) {
...
void open_output_file (char *fnam, char *tstr)
{
  ...
  if ((fout = fopen (outfnam,"w")) == NULL) { 
      error ("Cannot open output file ", outf); 
  }
  pagenum=0;
...
void open_index_file (const char *fnam)
{
  ...
  if ((findex = fopen (fnam,"w")) == NULL) { 
      error ("Cannot open index file: ", fnam);
  }
