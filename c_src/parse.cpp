...
int parse_meter_token(char* str, int* meter1, int* meter2, int* mflag, char* meter_top, int* dlen)
{
  ...
  if (str[0]=='C') {
    ...
    *mflag=3;
    ...
    q=strchr(meter_top,'/');
    if (!q) {
        std::cerr << "Cannot identify meter, missing /: " <<  str << std::endl;
      m1=m2=1;
      return 1;
    }
    *q='\0';
    ...
    q++;
    sscanf (q, "%d", &m2);
    if (m1*m2 == 0) { std::cerr << "Cannot identify meter: " << str << std::endl; }
    d=BASE/m2;
    if (d*m2 != BASE) std::cerr << "Meter not recognized: "  <<  str << std::endl;
    *meter1=m1;
    *meter2=m2;
    *dlen=EIGHTH;
