...
class Music:
    ...
    def draw_keysig(self, x, voice):
        ...
        n1 = self.u  # which symbol to start with
        n2 = self.v  # up to which symbol to go
        n3 = self.w  # draw neutrals instead starting from this one
        t = self.t  # type of symbol: sharp or flat

        if n2 > 7:
            print(f"+++ Keysig seems to have {n2} music ???")
            return False

        # set shift for accidentals in points(one tone = 3 points)
        yad = 0
        if voice.key.ktype == Key.BASS:
            yad = -6
        if voice.key.ktype == Key.ALTO:
            yad = -3
        if voice.key.ktype == constants.TENOR:
            yad = +3
        if voice.key.ktype == constants.SOPRANO:
            yad = +6
        if voice.key.ktype == constants.MEZZOSOPRANO:
            yad = -9
        if voice.key.ktype == constants.BARITONE:
            yad = -12
        if voice.key.ktype == constants.VARBARITONE:
            yad = -12
        if voice.key.ktype == constants.SUBBASS:
            yad = 0
        if voice.key.ktype == constants.FRENCHVIOLIN:
            yad = -6
...
def next_note(k, n, syms):  # Change the signature
    """
    Used only in set_sym_widths.
    The use is 0, len(syms)

    :param int k:
    :param int n:
    :param list syms: of Symbols
    :return int:
    """
    for i, sym in enumerate(syms[k+1:n]):
        if sym.type == NOTE or sym.type == REST or sym.type == BREST:
            return i
    return -1


def prec_note(k, n, symb):
    """

    :param int k:
    :param int n:
    :param list symb: of Symbols
    :return int:
    """
...
def followed_by_note(k, n, symb):
    ...
    for i in range(k + 1, n):
        if symb[i].type == INVISIBLE:
            break
        elif symb[i].type == NOTE or symb[i].type == REST or \
                symb[i].type == BREST:
            return i
        else:
            return -1
...
def xch(i, j):    # sub to exchange two integers
    """
...
# ----- print_vsyms: print music for all voices -----
void print_vsyms (void)
{
    ...
    for (i=0;i<nvoice;i++) {
        if (nvoice>1) 
            print("Voice <%s> (%d of %d)", voice[i].id,i,nvoice);
...
void mstave_deco (FILE *fp, int ip1, int ip2, float wid0, float hsys, float *htab, float botheight)
{
    ...
    for (iv=0;iv<nvoice;iv++) {
        ...
        if ((htop<hbot+0.001) && ((nv>1) and (voice[iv].bracket==1))) {
...
def output_music(fp):
    ...
    for voice in voices:
        voice.meter1 = voice.meter1
        voice.key1 = voice.key

    if not file_initialized and not epsf:
        pssubs.init_ps(fp, infostr)
        pssubs.init_page(fp)

    if len(voice) ==0:
        format.init_parse_params()
        return
    if verbose>=10:
        print_vsyms ()
    ...
    for voice in voices:
        if voice.draw:
            set_sym_chars(0,voice.nsym, symv)
            set_beams(0,voice.nsym, symv, is_tab_key(voice.key))
            if not is_tab_key(voice.key):
                set_stems(0,voice.nsym,symv)

            b = set_sym_times(0,len(voice.sym), symv, voice.meter0,
                              voice.timeinit)
            set_sym_widths(0, len(voice.nsym), symv, ivc)
            if ivc == ivc0:
                bnum=b
    barinit=bnum;
...
def check_selected(fp, xref_str, pat, sel_all, search_field, info):
    """
    :param fp:
    :param xref_str:
    :param pat:
    ...
    :param search_field:
    """
    if not info.do_this_tune:
        if is_selected(xref_str, len(pat), pat, sel_all, search_field):
            info.do_this_tune = True
            write_tunetop(fp)
